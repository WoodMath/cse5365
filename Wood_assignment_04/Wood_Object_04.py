...
class Object:
    ...
    def position(self, old_v_to_add, arr_add_to):
        ...
        v_to_add = copy.copy(old_v_to_add)
        ...
        if(i_current_vert_group < i_current_vert_group_count):
            if(self.start_index[i_current_vert_group] == None):
                ...
                if(i_found == None):                          ## No insert place found    
            ...
            for i_inc in range(i_current_vert_group_count,i_current_vert_group):
                self.start_index.append(None)                 ## To start indice array
    ...
    def testIndices(self):
        ...
        for i in range(len(self.lineIndices)):
            k = self.lineIndices[i]
            if(v_prev[0] > k[0]):
                b_Return = False
            if(v_prev[0] == k[0] and v_prev[0] >= k[0]):
                b_Return = False
